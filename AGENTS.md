...
- WinGet publishing is manual: run `.github/workflows/winget.yml` (workflow_dispatch) after a release. Requires secret `WINGET_TOKEN` (classic PAT, `public_repo`) and a fork of `microsoft/winget-pkgs`; the package identifier must already exist in winget-pkgs (first version is submitted manually).
