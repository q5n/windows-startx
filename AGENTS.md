...
- GitHub Release is automatic: `.github/workflows/release-x64.yml` fires on `v*` tags, builds `x86_64-pc-windows-msvc` and attaches `startx-vX.Y.Z-x64.zip`. Never hand-edit the version in `Cargo.toml` for a release — let the script do it.
- Manual x86 / arm64 releases: `.github/workflows/release.yml` and `.github/workflows/release-arm64.yml` are `workflow_dispatch` workflows. They build `i686-pc-windows-msvc` / `aarch64-pc-windows-msvc` and upload `startx-vX.Y.Z-x86.zip` / `startx-vX.Y.Z-arm64.zip` to the specified release (defaults to the latest release).
